...
progress$set(message = "Report in progress : parameters done", value = 0.2)
```
...
progress$set(message = "Report in progress : profil plot done", value = 0.3)
...
x<-df[df$Potential.contaminant=="",]
x <- x[,7:length(colnames(x))]
x <- log2(x)
x[x == -Inf]<-NA
x <- x[rowSums(is.na(x)) != ncol(x), ]

NbProt <- colSums(!is.na(x))

x <- data.frame(Sample=colnames(x),
                NbProt=NbProt)
...
progress$set(message = "Report in progress : Missed cleavages plot done", value = 0.4)
```

## 2.4 Normalisation and imputation  
...
progress$set(message = "Report in progress : Normalisation plot done", value = 0.5)
```
...
if(params$PCAparams$ShowSampleName==TRUE){
...
if(params$PCAparams$ShowSampleName==TRUE){
    ...
    g
progress$set(message = "Report in progress : PCA done", value = 0.6)    
```
...
if(MyStat==TRUE){
  ...
  ThresholdType<-params$Ttestparams$ThresholdType
  SignifThreshold<-params$Ttestparams$SignifThreshold
  FCThreshold<-params$Ttestparams$FCThreshold
  MyComparison<-params$Ttestparams$MyComp
  
  Imputation<-if(params$MyImputeMeth[1]=="YES"){paste0("Data were imputed by ", params$MyImputeMeth[2],".")}else{""}
}
```

# 3. Differential expression 

**`r if(MyStat==TRUE){Imputation}`** **`r if(MyStat==TRUE){StudWelch}`** tests **`r if(MyStat==TRUE){TypeTtest}`** were done on proteins showing at least **`r if(MyStat==TRUE){if(MyPaired==FALSE){paste(VVfilter,"valid values in each group and at least",TestpcFilter,"% of valid values in one condition")}else{paste(TestpcFilter,"% of paire of valid values")}}`** using log2(LFQ intensity).  
Significant threshold is **`r if(MyStat==TRUE){paste(ThresholdType,SignifThreshold, if(ThresholdType=="Qvalue"){"(Benjamini-Hoechberg)"},if(FCThreshold!=1){paste0(", FC>", FCThreshold," or <",round(1/FCThreshold,2))})}`**.  
...
params$GraphFC
progress$set(message = "Report in progress : FCplot done", value = 0.7)
```
...
for(i in 1:length(MyComparison)){
  ...
  n<-if(ThresholdType=="Pvalue"){0}else{1}
  ...
  x[x[,2]<SignifThreshold & x[,3]>log2(FCThreshold),"Type"]<-params$Volcanoparams$UpColor
  x[x[,2]<SignifThreshold & x[,3]<(-log2(FCThreshold)),"Type"]<-params$Volcanoparams$DownColor
  x[,1]<--log10(x[,1])
...
progress$set(message = "Report in progress : volcanoplot done", value = 0.8)
```
...
for (i in 1:length(MyComparison)){
  ...
  if(params$HMsignifparams$ShowApDisapeared==TRUE){
    MyListeProt<-(fdata()[,(MyColNumber+n)]<SignifThreshold
                  & (fdata()[,(MyColNumber+2)]<(-log2(FCThreshold ))
                     |fdata()[,(MyColNumber+2)]>log2(FCThreshold))
    )|(
  ...
  if(nrow(MyListeProt)>1){
    ...
    p[[i]]<-ComplexHeatmap::pheatmap(MyZscore,
                                     ...
                                     main = paste0("Differential proteins ",
                                                   MyComparison[i],
                                                   " (",ThresholdType,SignifThreshold ,if(FCThreshold!=1){paste0(", FC>",FCThreshold)}else{""},
                                                   ")"
                                     ),
                                     border_color = NA,
                                     angle_col="90"
...
progress$set(message = "Report in progress : heatmaps done", value = 0.9)
```
...
if(MyStat==TRUE){
  if(params$MyImputeMeth[1]=="YES"){
    x<-edata
    colnames(x)<-paste("Log2 imputed",colnames(x))
    x$RowNameX<-row.names(x)
    df<-merge(df,x,by.x="RowNamedf",by.y="RowNameX",all.x=TRUE)
  }
}  

if(MySoft=="Other" & MyStat==FALSE){
  DataToExport<-df[,c(2:MyNcol)] 
}else{
...
if(MyStat==TRUE){
  ...
  if(ThresholdType=="Pvalue"){PQ<-"Pval"}else{PQ<-"Qval"}
  MySignifVal<-paste0(SignifThreshold*100,"pc")
  if(FCThreshold==1){MySignifFC<-""}else{MySignifFC<-paste0("_FC",FCThreshold)}
...
MyRDS<-list(edata=edata,fdata=fdata,phenodata=phenodata,Pepdata=Pepdata, df=params$df)
...
progress$set(message = "Report in progress : export done", value = 1)
```
